/**
 * server.js — WhatsApp bot (broadcast + webhooks) — versão final
...
// Supabase (para broadcast por status)
...
// tabelas/campos (ajuste para sua base)
const ORDERS_TABLE      = process.env.ORDERS_TABLE || 'orders';
const ORDER_PHONE_FIELD = process.env.ORDER_PHONE_FIELD || 'customer_phone';
const ORDER_PAID_FIELD  = process.env.ORDER_PAID_FIELD || 'is_paid';

// segredos
const BROADCAST_SECRET = process.env.BROADCAST_SECRET || 'troque-este-segredo'; // x-api-key
const WEBHOOK_SECRET   = process.env.WEBHOOK_SECRET   || 'troque-este-segredo'; // x-webhook-secret
...
// instâncias
const INSTANCE_NAMES = (process.env.WPP_INSTANCES || 'instancia1').split(',').map(s => s.trim()).filter(Boolean);

// tag para broadcast em massa
const MASS_BROADCAST_LABEL = 'APP';
...
function createClient(name) {
  const client = new Client({
    ...
    puppeteer: {
      ...
      args: ['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage','--disable-web-security','--start-maximized','--disable-features=VizDisplayCompositor'],
      timeout: 120000
    },
    // versão de WA Web "travada" para estabilidade; troque/remova se preferir
    webVersionCache: {
      type: 'remote',
      remotePath: 'https://raw.githubusercontent.com/wppconnect-team/wa-version/main/html/2.2417.11.html'
    },
    qrMaxRetries: 3,
    takeoverOnConflict: true
  });

  client.on('qr', (qr) => { instStatus[name] = 'qr_code'; try { qrcode.generate(qr, { small:true }); } catch {}; pushLog(logs, { inst: name, evt: 'qr' }); });
  ...
  client.on('change_state', (st) => { if (st === 'CONNECTED') { instStatus[name] = 'online'; instNumber[name] = client?.info?.wid?.user || instNumber[name]; } });
  client.on('ready', () => { instStatus[name] = 'online'; instNumber[name] = client?.info?.wid?.user || instNumber[name]; pushLog(logs, { inst: name, evt: 'ready', wid: instNumber[name]||null }); });
  client.on('disconnected', (why) => { instStatus[name] = 'offline'; instNumber[name] = null; try { client.destroy(); } catch {} delete clients[name]; pushLog(logs, { inst: name, evt: 'disconnected', why }); });
...
function onIncoming(instName, client) {
  return async (msg) => {
    ...
    // (opcional) tratar códigos C123 aqui…
  };
}
...
INSTANCE_NAMES.forEach(name => {
  try {
    ...
    const t = setInterval(async () => {
      ...
      if (s === 'CONNECTED') { instStatus[name] = 'online'; instNumber[name] = clients[name]?.info?.wid?.user || instNumber[name]; pushLog(logs, { inst:name, evt:'reconciled', wid:instNumber[name]||null }); clearInterval(t); }
    }, 5000);
...
async function basicSend(req,res){
  ...
  } catch (e) { inQueue.delete(keyFor(req.body?.number||req.body?.to, req.body?.message)); res.status(500).json({ success:false, error:e.message }); }
}
...
/* ============================ Endpoint de configuração em lote (send-config) ============================ */
app.post('/api/send-config', async (req, res) => {
  try {
    const { data } = req.body;
    if (!data) return res.status(400).json({ erro: 'Dados não informados' });
    
    const config = JSON.parse(data);
    const { numeros, mensagens, interval = 3000, batchSize = 3, batchDelay = 5000 } = config;
    
    if (!numeros?.length || !mensagens?.length) {
      return res.status(400).json({ erro: 'Números e mensagens são obrigatórios' });
    }
