...
-- Handle user_role enum simplification
DO $$
BEGIN
  ...
  -- Update all rows to use new values
  UPDATE profiles SET role = 'admin'::text WHERE role::text IN ('master', 'admin');
  UPDATE profiles SET role = 'customer'::text WHERE role::text NOT IN ('master', 'admin');
