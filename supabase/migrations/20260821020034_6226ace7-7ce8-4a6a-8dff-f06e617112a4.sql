...
BEGIN
    ...
    customers_stats AS (
        SELECT 
            COUNT(*) as total,
            COUNT(*) FILTER (WHERE id IN (SELECT customer_id FROM public.orders WHERE is_paid = true)) as com_pedido_pago,
            COUNT(*) FILTER (WHERE id NOT IN (SELECT customer_id FROM public.orders WHERE is_paid = true)) as sem_pedido_pago
