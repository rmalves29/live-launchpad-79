...
BEGIN
  ...
  -- Gaps considerando apenas mensagens dos 4 tipos (item add, cancelado, pagamento, sem estoque)
  key_msgs AS (
    SELECT tid, ts
    FROM sent_flagged
  ...
  -- Gaps apenas de item_added
  item_msgs AS (
    SELECT tid, ts FROM sent_flagged WHERE is_item_added
  ),
  item_gaps AS (
  ...
  item_gap_agg AS (
    SELECT tid, AVG(gap_s) AS avg_item_gap
    FROM item_gaps
    WHERE gap_s IS NOT NULL AND gap_s <= 230*60
    GROUP BY tid
  ),
  -- Pico por minuto e por hora (todas mensagens enviadas)
  per_minute AS (
  ...
  per_hour AS (
    ...
    FROM sent_flagged
    GROUP BY tid, date_trunc('hour', ts)
  ),
  per_hour_agg AS (
  ...
  SELECT t.id, t.name,
    ...
    iga.avg_item_gap AS item_added_per_minute,
  ...
  LEFT JOIN item_gap_agg iga ON iga.tid = t.id
