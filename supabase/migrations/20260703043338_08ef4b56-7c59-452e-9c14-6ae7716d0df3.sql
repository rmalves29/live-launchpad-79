...
CREATE OR REPLACE FUNCTION public.admin_whatsapp_activity_metrics(p_from timestamptz, p_to timestamptz)
RETURNS TABLE(
  tenant_id uuid, tenant_name text,
  total_sent bigint,
  msgs_per_minute numeric, msgs_per_hour numeric,
  item_added_count bigint, item_added_per_minute numeric,
  order_cancelled_count bigint, payment_count bigint, out_of_stock_count bigint,
  group_msg_count bigint, received_private bigint,
  avg_gap_seconds numeric, last_gap_seconds numeric,
  disconnect_count bigint, avg_msgs_before_disconnect numeric, last_msgs_before_disconnect bigint
)
LANGUAGE plpgsql
