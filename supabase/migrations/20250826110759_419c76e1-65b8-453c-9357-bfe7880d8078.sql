-- Habilitar extensão http para fazer chamadas de API do trigger
CREATE EXTENSION IF NOT EXISTS http;

-- Atualizar função para chamar automaticamente a API do Melhor Envio
...
BEGIN
  ...
  IF NEW.is_paid = true AND (OLD.is_paid = false OR OLD.is_paid IS NULL) THEN
    ...
    IF v_envio_id IS NULL THEN
      ...
      BEGIN
        -- URLs do Supabase (usando as URLs hardcoded já que não podemos acessar variáveis de ambiente no trigger)
