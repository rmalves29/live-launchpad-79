-- Recriar a função para garantir precisão decimal no cálculo e tratamento de nulos
...
BEGIN
    ...
    SELECT json_build_object(
        ...
        'avg_shipping_time_hours', ROUND(AVG(EXTRACT(EPOCH FROM (shipped_at - paid_at))/3600)::numeric, 1) FILTER (WHERE paid_at IS NOT NULL AND shipped_at IS NOT NULL)
    ) INTO v_orders
    FROM orders
