  ...
  try {
    ...
    if (testResponse.ok) {
      ...
      return new Response(JSON.stringify({ 
        ...
        error: errorDetails,
        needs_oauth: true,
        oauth_url: `${baseUrl}/oauth/authorize?client_id=${integration.client_id}&redirect_uri=${encodeURIComponent('https://hxtbsieodbtzgcvvkeqx.supabase.co/functions/v1/callback-empresa?service=melhorenvio&action=oauth')}&response_type=code&state=${tenant_id}&scope=cart-read,cart-write,companies-read,companies-write,coupons-read,coupons-write,notifications-read,orders-read,products-read,products-destroy,products-write,purchases-read,shipping-calculate,shipping-cancel,shipping-checkout,shipping-companies,shipping-generate,shipping-preview,shipping-print,shipping-share,shipping-tracking,ecommerce-shipping,transactions-read,users-read,users-write,webhooks-read,webhooks-write,webhooks-delete`
      }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
