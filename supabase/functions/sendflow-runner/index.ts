...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch(`${supabaseUrl}/functions/v1/sendflow-process`, {
      ...
      body: JSON.stringify({ job_id: body.job_id, tenant_id: body.tenant_id }),
    });

    const text = await response.text();
    let payload: any = null;
    try { payload = text ? JSON.parse(text) : null; } catch { payload = { raw: text.substring(0, 300) }; }
    ...
    console.error("[sendflow-runner] Error:", error.message);
