...
export default function CategoriasManagerDialog({ open, onOpenChange, onChanged }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error(e);
      toast({ title: 'Erro', description: e?.message || 'Falha ao carregar categorias', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Use categorias para agrupar produtos (ex: "Anéis", "Promo") e aplicar promoções em lote.
            </DialogDescription>
          </DialogHeader>
