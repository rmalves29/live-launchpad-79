          ...
          <div className="flex items-center justify-between">
            <NavLink to="/" className="flex items-center gap-3 group">
              <img
                src={cartzyLogo}
                alt="Cartzy"
                className="h-11 md:h-[3.3rem] w-auto object-contain transition-transform duration-300 group-hover:scale-105"
              />
            </NavLink>
            ...
            <div className="flex items-center gap-0.5 bg-muted/50 backdrop-blur-sm rounded-xl p-1 border border-border/30">
              {navItems.map((item) => (
                <NavLink
                  ...
                  to={item.path}
                  className={({ isActive }) =>
                    `px-4 py-2.5 rounded-lg text-base font-medium transition-all duration-200 whitespace-nowrap ${
                      isActive
                        ? 'bg-primary text-primary-foreground shadow-glow-sm'
