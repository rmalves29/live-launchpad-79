...
class TenantManager {
  ...
  async createClient(tenant) {
    ...
    // Inicializar
    console.log(`🔄 ${tenant.name}: Inicializando WhatsApp Web...`);
    console.log(`⏰ Aguarde o QR Code (até 120s)...\n`);

    // Timeout de segurança
    const initTimeout = setTimeout(() => {
      console.error(`\n⏱️ TIMEOUT: ${tenant.name} - QR Code não gerado em 120s`);
      console.error(`\n💡 Soluções:`);
      console.error(`   1. Reinicie o computador`);
      console.error(`   2. Feche outros Chrome/Edge`);
      console.error(`   3. Desative antivírus\n`);
    ...
    try {
      ...
      console.log(`✅ ${tenant.name}: Cliente inicializado com sucesso`);
    } catch (error) {
      clearTimeout(initTimeout);
      console.error(`❌ ${tenant.name}: Erro ao inicializar:`, error.message);
